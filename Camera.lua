...
local function StopLivestream()
	local pidFile = io.open("Livestream.pid", "r")
...
local function StartLivestream(from, port)
	...
	os.execute([[rpicam-vid -t 0 -n --codec libav --libav-format mpegts --width 1280 --height 720 --framerate 30 --bitrate 2000000 --intra 15 -o "udp://]]..from:GetRemoteDetails()..":"..port..[[?pkt_size=1316&buffer_size=65535" >/dev/null 2>&1 & echo $! > Livestream.pid]])
end
