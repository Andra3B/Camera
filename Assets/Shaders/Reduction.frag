...
vec4 effect(vec4 colour, Image inputFrame, vec2 imagePosition, vec2 screenPosition) {
	vec2 base = (imagePosition * 2.0) - TexelSize;

	vec4 topLeft = Texel(inputFrame, base + vec2(0.5, 0.5)*TexelSize);
	vec4 topRight = Texel(inputFrame, base + vec2(1.5, 0.5)*TexelSize);
	vec4 bottomLeft = Texel(inputFrame, base + vec2(0.5, 1.5)*TexelSize);
	vec4 bottomRight = Texel(inputFrame, base + vec2(1.5, 1.5)*TexelSize);
